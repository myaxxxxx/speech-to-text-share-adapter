...
from .prefix_transformer import PrefixTransformerDecoder, PrefixTransformerEncoder, PrefixLayers
...
@register_model("deltalm_transformer_prefix")
class DeltalmTransformerModel(FairseqEncoderDecoderModel):
    ...
    def build_decoder(cls, args, task, embed_tokens):
        ...
        decoder = DeltaLMDecoder(TransformerConfig.from_namespace(args), task.target_dictionary, embed_tokens, decoder_prefixes=decoder_prefixes)
        if args.frozen_plm:
...
class DeltalmTransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args, dictionary=None, embed_tokens=None, return_fc=False):
        ...
        if args.layer_prefix and args.encoder_prefix_length != 0:
            self.prefixes = nn.ModuleList([PrefixLayers(args.encoder_prefix_length, args.encoder_embed_dim,
                                                           args.reparametrize_prefix, args.reparametrize_dim)
...
@register_model_architecture(model_name="deltalm_transformer_prefix", arch_name="deltalm_transformer_prefix")
def base_architecture(args):
    # subsampler
...
# adda prefix
    ...
    args.layer_prefix =getattr(args, "layer_prefix", True)
